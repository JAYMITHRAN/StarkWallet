...
export function ExpenseForm({ currentBalance, onSubmitted }) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: async () => {
            ...
            show({ tone: "success", title: "Cash out saved", description: "Dashboard, history and summary are in sync." });
            onSubmitted?.();
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Card, { className: "space-y-4 p-4", children: [isLowBalance && (_jsxs("div", { className: "rounded-xl border border-warning/40 bg-warning/10 px-3 py-2 text-sm text-warning", role: "status", children: ["Low balance warning \u2014 only ", formatCurrency(remainingBalance), " remains."] })), _jsxs("div", { className: "rounded-2xl border border-border/80 bg-white/5 p-4", children: [_jsxs("div", { className: "flex items-center gap-2 text-sm text-text-muted", children: [_jsx(Sparkles, { className: "h-4 w-4 text-accent", "aria-hidden": true }), "Smart suggestions for ", selectedCategory.toLowerCase()] }), _jsx("div", { className: "mt-3 flex flex-wrap gap-2", children: suggestions.map((suggestion) => (_jsx("button", { type: "button", className: "rounded-full border border-border bg-surface px-3 py-1 text-xs text-text-muted hover:bg-white/5", onClick: () => setValue("reason", suggestion, { shouldDirty: true }), children: suggestion }, suggestion))) })] }), _jsxs("form", { className: "flex flex-col gap-4", onSubmit: handleSubmit(onSubmit), noValidate: true, children: [_jsx(FormField, { label: "Amount", htmlFor: "amount", error: errors.amount?.message, children: _jsx(Input, { id: "amount", type: "number", step: "0.01", inputMode: "decimal", placeholder: "0.00", autoFocus: true, ...register("amount") }) }), _jsx(FormField, { label: "Category", htmlFor: "category", error: errors.category?.message, children: _jsx("select", { id: "category", className: "h-11 w-full rounded-xl border border-border bg-surface px-4 text-sm text-text", ...register("category"), onChange: (event) => {
                                        const nextValue = event.target.value;
                                        setSelectedCategory(nextValue);
                                        setValue("category", nextValue, { shouldDirty: true });
                                    }, children: categoryOptions.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value))) }) }), _jsx(FormField, { label: "Reason", htmlFor: "reason", error: errors.reason?.message, children: _jsx(Input, { id: "reason", placeholder: "Enter a reason or pick a suggestion", ...register("reason") }) }), _jsx(FormField, { label: "Notes (optional)", htmlFor: "note", error: errors.note?.message, children: _jsx(Input, { id: "note", placeholder: "Optional note", ...register("note") }) }), _jsx(FormField, { label: "Date & Time", htmlFor: "occurredAt", error: errors.occurredAt?.message, children: _jsxs("div", { className: "flex items-center gap-2 rounded-xl border border-border bg-surface px-3", children: [_jsx(CalendarDays, { className: "h-4 w-4 text-text-muted", "aria-hidden": true }), _jsx(Input, { id: "occurredAt", type: "datetime-local", ...register("occurredAt") })] }) }), _jsxs("div", { className: "rounded-2xl border border-border/80 bg-surface/70 p-4", children: [_jsxs("div", { className: "flex items-center justify-between text-sm", children: [_jsx("span", { className: "text-text-muted", children: "Current balance" }), _jsx("span", { className: "font-semibold text-text", children: formatCurrency(currentBalance) })] }), _jsxs("div", { className: "mt-2 flex items-center justify-between text-sm", children: [_jsx("span", { className: "text-text-muted", children: "Expense" }), _jsx("span", { className: "font-semibold text-danger", children: formatCurrency(watchedAmount) })] }), _jsxs("div", { className: "mt-2 flex items-center justify-between text-sm", children: [_jsx("span", { className: "text-text-muted", children: "Remaining" }), _jsx("span", { className: cn("font-semibold", remainingBalance < 0 ? "text-danger" : "text-success"), children: formatCurrency(remainingBalance) })] })] }), _jsxs(Button, { type: "submit", variant: "black", size: "lg", isLoading: isSubmitting || mutation.isPending, children: [_jsx(ArrowUpCircle, { className: "h-4 w-4", "aria-hidden": true }), "Save Expense"] })] })] }), showConfirm && (_jsx("div", { className: "fixed inset-0 z-[60] flex items-center justify-center bg-black/70 px-4", children: _jsxs("div", { className: "glass-panel max-w-md rounded-2xl p-5", children: [_jsx("p", { className: "text-base font-semibold text-text", children: "This expense exceeds your current balance." }), _jsx("p", { className: "mt-2 text-sm text-text-muted", children: "You can continue saving it, but your balance will go negative." }), _jsxs("div", { className: "mt-4 flex justify-end gap-2", children: [_jsx(Button, { variant: "ghost", onClick: () => setShowConfirm(false), children: "Cancel" }), _jsx(Button, { variant: "black", onClick: confirmContinue, children: "Continue" })] })] }) }))] }));
}
