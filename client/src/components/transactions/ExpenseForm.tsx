...
export function ExpenseForm({ currentBalance, onSubmitted }: ExpenseFormProps) {
      ...
      <Card className="space-y-4 p-4">
        ...
        <div className="rounded-2xl border border-border/80 bg-white/5 p-4">
          <div className="flex items-center gap-2 text-sm text-text-muted">
            ...
            Smart suggestions for {selectedCategory.toLowerCase()}
          </div>
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <FormField label="Reason" htmlFor="reason" error={errors.reason?.message}>
            <Input id="reason" placeholder="Enter a reason or pick a suggestion" {...register("reason")} />
          </FormField>
          ...
          <FormField label="Date & Time" htmlFor="occurredAt" error={errors.occurredAt?.message}>
          ...
          <div className="rounded-2xl border border-border/80 bg-surface/70 p-4">
            ...
            <div className="mt-2 flex items-center justify-between text-sm">
              ...
              <span className={cn("font-semibold", remainingBalance < 0 ? "text-danger" : "text-success")}>{formatCurrency(remainingBalance)}</span>
            </div>
          </div>
          <Button type="submit" variant="black" size="lg" isLoading={isSubmitting || mutation.isPending}>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 px-4">
          <div className="glass-panel max-w-md rounded-2xl p-5">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <Button variant="black" onClick={confirmContinue}>
                Continue
              </Button>
            </div>
          </div>
